#!/bin/bash
# Windows 部署路徑登記表（WSL 側，/mnt/c 版本）
#
# 所有 WSL 腳本引用此檔案的變數，禁止自行硬寫 /mnt/c 路徑。
# 此檔案必須與 deploy-paths.ps1 保持一致。
#
# 用法: source ~/dotfiles/windows/deploy-paths.sh

WIN_USER="duofilm"
WIN_HOME="/mnt/c/Users/${WIN_USER}"
WIN_LOCALAPPDATA="${WIN_HOME}/AppData/Local"

DEPLOY_LHM_DIR="${WIN_LOCALAPPDATA}/LibreHardwareMonitor"
DEPLOY_WIN_STATS_DIR="${WIN_LOCALAPPDATA}/win-stats-mqtt"
DEPLOY_WIN_STATS_MAIN="${DEPLOY_WIN_STATS_DIR}/push-win-stats.ps1"
DEPLOY_WIN_STATS_RUNNER="${DEPLOY_WIN_STATS_DIR}/run-win-stats.vbs"

DEPLOY_OVERLAY_DIR="${WIN_LOCALAPPDATA}/claude-overlay"
DEPLOY_OVERLAY_MAIN="${DEPLOY_OVERLAY_DIR}/claude-overlay.exe"

DEPLOY_SD_PLUGIN="${WIN_HOME}/com.duofilm.claude-monitor.sdPlugin"

DEPLOY_CLAUDE_MONITOR_DIR="${WIN_LOCALAPPDATA}/claude-monitor"
DEPLOY_CLAUDE_MONITOR_SIDECAR="${DEPLOY_CLAUDE_MONITOR_DIR}/sidecar.js"
DEPLOY_CLAUDE_MONITOR_RUNNER="${DEPLOY_CLAUDE_MONITOR_DIR}/run-sidecar.ps1"
DEPLOY_CLAUDE_MONITOR_HIDDEN_RUNNER="${DEPLOY_CLAUDE_MONITOR_DIR}/run-sidecar.vbs"
DEPLOY_CLAUDE_MONITOR_STATE="${DEPLOY_CLAUDE_MONITOR_DIR}/state.json"
DEPLOY_CLAUDE_MONITOR_LOG="${DEPLOY_CLAUDE_MONITOR_DIR}/sidecar.log"
